...
def load_migration_module():
    module_path = Path(__file__).resolve().parent.parent / "scripts" / "migrate_health_db.py"
...
def create_target_db(path: Path):
    schema = (Path(__file__).resolve().parent.parent / "backend" / "schema.sql").read_text()
...
def test_migrate_health_db_imports_food_entries_and_is_idempotent(tmp_path: Path):
    ...
    try:
        ...
        source_conn.execute(
            """INSERT INTO food_entries
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            ("2026-02-25", "Lunch", "Turkey sandwich", 540, 38, "legacy import", "2026-02-25 18:30:00"),
        )
        source_conn.commit()
    finally:
        source_conn.close()
    ...
    second_inserted, second_skipped = migration.migrate_food_entries(str(source_db), str(target_db))
    ...
    try:
        row = target_conn.execute(
            "SELECT recorded_date, meal_type, name, calories, protein_g, notes FROM food_entries"
        ).fetchone()
    finally:
        target_conn.close()
