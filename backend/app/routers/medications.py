...
@router.get("/")
def get_medications(active_only: int = 1):
    conn = get_db()
    try:
        if active_only:
            rows = conn.execute(
...
def create_medication(entry: MedicationCreate):
    conn = get_db()
    try:
        cur = conn.execute(
...
def update_medication(medication_id: int, patch: MedicationPatch):
    conn = get_db()
    try:
        ...
        values = list(normalized.values()) + [medication_id]
        conn.execute(
